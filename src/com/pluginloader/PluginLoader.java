		...
		if (fc.showOpenDialog(null) == JFileChooser.APPROVE_OPTION) {
			...
			try {
				...
				while (e.hasMoreElements()) {
					...
					if (jarElement.isDirectory() || !jarElement.getName().endsWith(".class")) {
						continue;
					}
					String className = jarElement.getName().substring(0, jarElement.getName().length() - 6);
					className = className.replace('/', '.');
					if (className.contains("Impl")) {
						Class<?> c = cl.loadClass(className);
						Constructor<?> constructeur = c.getDeclaredConstructor(mainView.getClass());
				...
				System.out.println("Instanciation Exception : " + e1.getMessage());
			} catch (IllegalAccessException e1) {
				System.out.println("Illegal Access Exception : " + e1.getMessage());
			} catch (InvocationTargetException e1) {
				System.out.println("Invocation Target Exception : " + e1.getMessage());
